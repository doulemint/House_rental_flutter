...
import 'package:house_rental/widgets/common_check_button.dart';
        ...
        children: list
            .map((f) => GestureDetector(
                  onTap: () {
                    setState(() {
                      list = list
                          .map((innerITem) => innerITem == f
                              ? RoomApplianceItem(
                                  f.title, f.iconPoint, f.isChecked)
                              : innerITem)
                          .toList();
                    });
                    if (widget != null) {
                  ...
                  child: Container(
                    ...
                    child: Column(children: <Widget>[
                      Icon(
                        IconData(f.iconPoint, fontFamily: Config.CommonIcon),
                        size: 40.0,
                      ),
                      Padding(
                          padding: const EdgeInsets.all(10.0),
                          child: Text(f.title)),
                      CommonCheckButton(f.isChecked)
                    ]),
                  ),
                ))
            .toList(),
